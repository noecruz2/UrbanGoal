...
(async () => {
  try {
    ...
    // Auto-migración robusta: Asegurar que las columnas necesarias existan
    setTimeout(async () => {
      try {
        console.log('[DB-Migration] Verificando columnas en tabla "products"...');
        
        // Columna 'type'
        try {
          await pool.query("ALTER TABLE products ADD COLUMN type VARCHAR(50) DEFAULT 'tenis';");
          console.log('[DB-Migration] Columna "type" añadida.');
        } catch (err) {
        ...
        try {
          ...
          console.log('[DB-Migration] Columna "isActive" añadida.');
        } catch (err) {
          if (err.code !== 'ER_DUP_FIELDNAME' && !err.message.includes('Duplicate column')) throw err;
        }

        console.log('[DB-Migration] Migraciones completadas exitosamente.');
      } catch (err) {
...
app.post('/api/products', verifyAuth, verifyAdmin, uploadMultipleImages, async (req, res) => {
  ...
  try {
    ...
    const isActive = req.body.isActive === 'false' || req.body.isActive === false || req.body.isActive === 0 || req.body.isActive === '0' ? 0 : 1;

    await pool.query(
      'INSERT INTO products (id, name, brand, price, originalPrice, images, description, sizes, category, type, featured, isActive) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [id, name, brand, numericPrice, numericOriginalPrice, JSON.stringify(images), description, JSON.stringify(parsedSizes), category, type || 'tenis', isFeatured, isActive]
    );
    res.status(201).json({ 
      id, 
      name, 
...
app.put('/api/products/:id', verifyAuth, verifyAdmin, uploadMultipleImages, async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE products SET name = ?, brand = ?, price = ?, originalPrice = ?, images = ?, description = ?, sizes = ?, category = ?, type = ?, featured = ?, isActive = ? WHERE id = ?',
      [name, brand, numericPrice, numericOriginalPrice, JSON.stringify(images), description, JSON.stringify(parsedSizes), category, type || 'tenis', isFeatured, isActive, paramId]
    );
    res.status(200).json({ 
      id: paramId,
      name, 
